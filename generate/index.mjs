...
import { pokemon as data } from "./data.mjs";

const GET_NEW_DATA = true;
...
const generate = async () => {
  ...
  for (const pokemon of pokedex) {
    console.log(pokemon);
    if (GET_NEW_DATA && data[`pokemon:${pokemon}`]) {
      pokemonLISTDATA[`pokemon:${pokemon}`] = data[`pokemon:${pokemon}`];
      continue;
    }
    await P.getPokemonByName(pokemon) // with Promise
      .then(async (response) => {
        ...
        pokemonLISTDATA[`pokemon:${response.name}`] = {
          ...
          icon: `textures/items/pokemon/${response.name}`,
          stats: stats,
